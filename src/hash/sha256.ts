 ...
 * @param {number} shift
 */
const rotr = (w: number, shift: number) => (w >>> shift) | (w << (32 - shift))

/**
 * Helper for SHA-224 & SHA-256. See 4.1.2.
 * @param {number} x
 */
const sum0to256 = (x: number) => rotr(x, 2) ^ rotr(x, 13) ^ rotr(x, 22)

/**
 * Helper for SHA-224 & SHA-256. See 4.1.2.
 * @param {number} x
 */
const sum1to256 = (x: number) => rotr(x, 6) ^ rotr(x, 11) ^ rotr(x, 25)

/**
 * Helper for SHA-224 & SHA-256. See 4.1.2.
 * @param {number} x
 */
const sigma0to256 = (x: number) => rotr(x, 7) ^ rotr(x, 18) ^ x >>> 3

/**
 * Helper for SHA-224 & SHA-256. See 4.1.2.
 * @param {number} x
 */
const sigma1to256 = (x: number) => rotr(x, 17) ^ rotr(x, 19) ^ x >>> 10
 ...
 * @param {Uint8Array} data
 */
export const digest = (data: Uint8Array) => new Hasher().digest(data)